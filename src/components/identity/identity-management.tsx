...
export function IdentityManagement() {
  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
      <Card className="overflow-hidden"><div className="border-b border-border px-4 py-3"><h2 className="font-semibold">Pessoas com acesso</h2><p className="text-sm text-subdued">Funcionários ativos e a loja atribuída a cada um.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-left text-sm"><thead className="bg-muted text-xs uppercase text-subdued"><tr><th className="px-4 py-3">Nome</th><th className="px-4 py-3">E-mail</th><th className="px-4 py-3">Perfil</th><th className="px-4 py-3">Loja</th><th className="px-4 py-3">Situação</th></tr></thead><tbody className="divide-y divide-border">{(usersQuery.data ?? []).map((user) => <tr key={`${user.id}-${user.roleId}`}><td className="px-4 py-3 font-medium">{user.name}</td><td className="px-4 py-3 text-subdued">{user.email}</td><td className="px-4 py-3">{user.roleName}</td><td className="px-4 py-3">{user.branchName ?? "Todas as lojas"}</td><td className="px-4 py-3"><Badge>{user.status === "ACTIVE" ? "Ativo" : user.status}</Badge></td></tr>)}{usersQuery.data?.length === 0 ? <tr><td className="px-4 py-6 text-center text-subdued" colSpan={5}>Nenhum usuário cadastrado.</td></tr> : null}</tbody></table></div></Card>
