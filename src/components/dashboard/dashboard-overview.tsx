...
export function DashboardOverview() {
  ...
  const metrics = [
    { label: "Faturamento dos últimos 30 dias", value: money(data?.metrics.revenue ?? 0), hint: "Calculado com as vendas registradas", icon: ArrowUpRight },
    { label: "Lucro bruto dos últimos 30 dias", value: money(data?.metrics.grossProfit ?? 0), hint: `${(data?.metrics.margin ?? 0).toFixed(1)}% de margem sobre as vendas`, icon: Banknote },
    { label: "Despesas pendentes", value: money(data?.metrics.pendingExpenses ?? 0), hint: "Lançamentos ainda não pagos", icon: ArrowDownRight },
    { label: "Produtos com estoque baixo", value: String(data?.metrics.lowStock ?? 0), hint: "Produtos que precisam de reposição", icon: PackageSearch }
  ];
