...
export async function GET() {
  try {
    ...
    const [sales, products, pendingExpenses, recentExpenses] = await Promise.all([
      prisma.sale.findMany({ where: { tenantId, status: "COMPLETED", soldAt: { gte: thirtyDaysAgo } }, include: { customer: { select: { name: true } }, items: true }, orderBy: { soldAt: "desc" } }),
      prisma.product.findMany({ where: { tenantId, status: "ACTIVE" }, select: { stockQuantity: true, minStock: true } }),
      prisma.financialEntry.aggregate({ where: { tenantId, type: "EXPENSE", status: "PENDING" }, _sum: { amount: true } }),
      prisma.financialEntry.findMany({ where: { tenantId, type: "EXPENSE", status: "PAID", paidAt: { gte: sevenDaysAgo } }, orderBy: { paidAt: "asc" } })
    ]);
    ...
    const upcomingExpenses = await prisma.financialEntry.findMany({ where: { tenantId, type: "EXPENSE", status: "PENDING" }, orderBy: { dueDate: "asc" }, take: 5 });
    return ok({ metrics: { revenue, grossProfit: revenue - cost, margin: revenue ? ((revenue - cost) / revenue) * 100 : 0, pendingExpenses: number(pendingExpenses._sum.amount), lowStock: products.filter((product) => number(product.minStock) > 0 && number(product.stockQuantity) <= number(product.minStock)).length }, cashFlow, topProducts: Array.from(productTotals, ([name, quantity]) => ({ name, quantity })).sort((a, b) => b.quantity - a.quantity).slice(0, 5), latestSales: sales.slice(0, 5).map((sale) => ({ id: sale.id, code: sale.code, customerName: sale.customer?.name ?? "Consumidor final", paymentMethod: paymentLabels[sale.paymentMethod] ?? sale.paymentMethod, total: number(sale.total), soldAt: sale.soldAt.toISOString() })), upcomingExpenses: upcomingExpenses.map((entry) => ({ id: entry.id, description: entry.description, amount: number(entry.amount), dueDate: entry.dueDate.toISOString() })) });
