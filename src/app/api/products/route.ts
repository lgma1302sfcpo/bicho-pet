...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await productService.listProducts(session.user.currentTenantId, filters);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const product = await productService.createProduct(session.user.currentTenantId, input);
