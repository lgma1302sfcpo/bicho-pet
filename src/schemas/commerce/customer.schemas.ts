...
export const createCustomerSchema = z.object({
  ...
  city: optionalText,
  cityCode: optionalDigits.refine((value) => !value || value.length === 7, "O codigo do municipio deve possuir sete numeros."),
  state: optionalText.refine((value) => !value || value.length === 2, "Selecione um estado."),
  zipCode: optionalDigits.refine((value) => !value || value.length === 8, "O Codigo de Enderecamento Postal deve possuir oito numeros."),
