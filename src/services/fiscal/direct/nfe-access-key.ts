...
export function nfeCheckDigit(base43: string) {
  if (!/^\d{43}$/.test(base43)) throw new Error("A base da chave de acesso deve possuir 43 numeros.");
  let weight = 2;
  let total = 0;
...
export function buildNfeAccessKey(input: {
  ...
  if (cnpj.length !== 14) throw new Error("O Cadastro Nacional da Pessoa Juridica deve possuir 14 numeros.");
  if (!Number.isInteger(input.series) || input.series < 0 || input.series > 999) throw new Error("A serie fiscal deve estar entre zero e 999.");
  if (!Number.isInteger(input.number) || input.number < 1 || input.number > 999_999_999) throw new Error("O numero fiscal deve estar entre um e 999999999.");
  const date = saoPauloParts(input.issuedAt);
  const numericCode = input.numericCode ?? pad(randomInt(1, 100_000_000), 8);
  if (!/^\d{8}$/.test(numericCode)) throw new Error("O codigo numerico deve possuir oito numeros.");
