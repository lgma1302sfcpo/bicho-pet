...
const paymentLabels: Record<string, string> = { CASH: "Dinheiro", PIX: "PIX", CREDIT_CARD: "Cartão de Crédito", DEBIT_CARD: "Cartão de Débito", STORE_CREDIT: "Fiado", VOUCHER: "Vale", MIXED: "Pagamento múltiplo" };
...
export async function createDirectDanfe(input: { request: FiscalProviderRequest; accessKey: string; protocol?: string; qrCodeUrl?: string | null; pendingContingency?: boolean }) {
  ...
  const headerLeft = consumer ? 29 : left;
  const headerWidth = consumer ? right - headerLeft : contentWidth;
