import type { FiscalProvider, FiscalProviderRequest, FiscalServiceStatus, FiscalVoidRequest } from "@/interfaces/fiscal/fiscal-provider.interface";
...
function responseRoot(xml: string, localName: string) {
  ...
  if (fault) throw new AppError(`A Secretaria da Fazenda recusou a requisicao SOAP: ${fault}`, "SEFAZ_SOAP_FAULT", 502);
  ...
  if (!root) throw new AppError(`Retorno da Secretaria da Fazenda sem ${localName}.`, "SEFAZ_UNEXPECTED_RESPONSE", 502);
  return root;
}
