...
import { isValidCpfOrCnpj } from "@/lib/brazilian-documents";
...
function destinationXml(request: FiscalProviderRequest) {
  ...
  if (!tag || !isValidCpfOrCnpj(document)) throw new AppError("O documento do cliente deve ser um Cadastro de Pessoa Fisica ou Cadastro Nacional da Pessoa Juridica valido.", "FISCAL_CUSTOMER_DOCUMENT_INVALID", 422);
...
export function buildNfeXml(request: FiscalProviderRequest, privateKeyPem: string) {
  ...
  const productsTotal = request.sale.items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0);
