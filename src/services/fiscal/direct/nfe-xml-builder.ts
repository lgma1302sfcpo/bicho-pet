...
export function buildNfeXml(request: FiscalProviderRequest, privateKeyPem: string) {
  ...
  const card = ["CREDIT_CARD", "DEBIT_CARD"].includes(request.sale.paymentMethod) ? "<card><tpIntegra>2</tpIntegra></card>" : "";
