...
export function buildCustomerWhatsAppMessage(
  ...
  if (type === "REACTIVATION") {
    ...
    return `Olá, ${name}! Aqui é da Bicho Pet. ${history} e sentimos sua falta. Temos novidades para você e seu pet. Posso te mostrar?`;
  }
  ...
  if (type === "LOYALTY") {
    ...
    return `Olá, ${name}! Aqui é da Bicho Pet. ${purchases} Estamos à disposição para cuidar do seu pet. Precisa de alguma coisa?`;
  }

  if (type === "BIRTHDAY") {
    return `Olá, ${name}! A equipe da Bicho Pet deseja um feliz aniversário! Preparamos uma mensagem especial para celebrar esse dia com você. Posso te contar?`;
  }

  if (customer.purchaseCount === 0) {
    return `Olá, ${name}! Aqui é da Bicho Pet. Vimos que você ainda não fez sua primeira compra conosco. Será um prazer ajudar a encontrar o que seu pet precisa. Como podemos ajudar?`;
  }

  return `Olá, ${name}! Aqui é da Bicho Pet. Estamos entrando em contato para saber se você e seu pet precisam de alguma coisa. Como podemos ajudar?`;
}
