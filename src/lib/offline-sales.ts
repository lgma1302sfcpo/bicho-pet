...
export type OfflineProduct = { id: string; name: string; code?: string | null; sku?: string | null; barcode?: string | null; salePrice: number; stockQuantity: number; unit: string; status: string };
export type PendingOfflineSale = { id: string; scope: string; createdAt: string; total: number; payload: CreateSaleDTO; status: "PENDING" | "ERROR"; error?: string };
type Catalog = { scope: string; updatedAt: string; cashRegisterSessionId: string | null; products: OfflineProduct[] };

const DB_NAME = "erp-offline-sales-v1";
